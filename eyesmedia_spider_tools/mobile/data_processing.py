...
class PttProcessing(object):
    ...
    def processing(self, filename, to_csv=True, to_mongo=True):
...
class MobileProcessing(object):
    ...
    def processing(self, filename, to_mongo=True):
        ...
        if to_mongo:
            self.collection = mongo_conn()
            logger.info("Ready to write processed mobile01 data to mongo. collection name:{}".format(mongo_info["collection"]))
            insert_mongo(res_dict, self.collection)
...
#     filename = "/Users/Chives/eyes/eyesmedia_spider_tools/mobile/data/mobile01_living_1.json"
#     prc = MobileProcessing()
#     res = prc.processing(filename,to_mongo=True)